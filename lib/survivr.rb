...
def phase_three
  ...
  total_remaining_members @merge_tribe

  puts
  end
end

def total_remaining_members(tribe)
  tribe.members.each_with_index do |member, i|
    print member.name.capitalize.pink + ', ' if i < tribe.members.length - 1
    puts member.name.capitalize.pink if i == tribe.members.length - 1
  end
end
